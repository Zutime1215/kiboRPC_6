...
def cropImgCalc(pts):
    ...
    E = [pth[0] - ltrl(1.3), pth[1] + ltrl(11)]
...
def main():
    ...
    model = YOLO(f"{model_name}/{model_name}.pt")
    ...
    cv2.fillPoly(img, pts=np.array([[nmZero, pts[1], pts[2], pts[4]]], dtype=np.int32), color=(0, 0, 0)) # fill Aruco Marker

    pts = np.array([pts[0], pts[1], pts[2], pts[3]])
    x, y, w, h = cv2.boundingRect(pts)
    cropped_img = img[y:y+h, x:x+w]

    results = model(cropped_img, conf=conf)
    results[0].show()
...
if __name__ == '__main__':
    ...
    img_path = "../../item_detect/test_items_2/1.jpg"
    model_name = "yolo11s_300_480_grayscale"
    conf = 0.3
